...
SCHEMAS = {
    ...
    'credit_note': 'schemas/nota_credito.xsd',
...
class DocumentXML(object):
    ...
    def send_xml_sri(self, document, api_sri, timeout):
        ...
        self.logger.info('Enviando documento ws SRI')
        buf = StringIO()
        buf.write(str(document))
        buffer_xml = base64.encodestring(str(document).encode('utf-8')).decode().replace('\n', '').replace('\\', '')
        try:
    ...
    def request_authorization(self, access_key, url_api):
        ...
        self.logger.debug("Respuesta Autorizacion Comprobante SRI {}".format(result))
