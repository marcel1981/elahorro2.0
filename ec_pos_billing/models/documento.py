...
class ElectronicDocument(models.AbstractModel):
    ...
    def get_access_key(self, name):
        ...
        series = "{}{}".format(
            config_pos.number_establishment.number_establish, config_pos.code_emission
        )
        number = self.get_secuencial()
        code_num = str(self.id)
        type_emission = config_pos.type_emision
        self._logger.info(
            "info access ke {} {} {} {}".format(
                type_comp, ruc, company.env_type, series
            )
        )
        access_key = (
    ...
    def _payment_methods(self, order):
        ...
        for pay_met in order.statement_ids:
            ...
            if pay_met.journal_id.code == "01":
                ...
                line_pay = {"formaPago": "01", "total": "{:.2f}".format(pay_met.amount)}
        ...
        if amount_cash > 0:
            line_pay = {"formaPago": "01", "total": "{:.2f}".format(amount_cash)}
            pay_method.append(line_pay)
        ...
        for tax in self.lines:
            ...
            if tax.tax_ids_after_fiscal_position.name == "IVA 12":
                ...
                node_tax_12 = {
                    ...
                    "codigoPorcentaje": utils.tabla17[
                        tax.tax_ids_after_fiscal_position.name
                    ],
    ...
    def send_mail_to_client(self):
        ...
        context["action_id"] = ir_model_data.get_object_reference(
            "addon", "object_acction"
        )[1]
        # Obtain Id template
        template_id = ir_model_data.get_object_reference(
            "addon", "email_template_invoice"
        )[1]
        mail = email_tmp_obj.with_context(context)
        self.pool.get("email.template").send_mail(
            self._cr, self._uid, template_id, self.id, force_send=True, context=context
        )
        return True
