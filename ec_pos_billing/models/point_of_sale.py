...
class PosInvoice(models.Model):
    ...
    establishment = fields.Many2one(string='Establecimiento', related='point_of_sale.number_establishment')
...
class PosOrder(models.Model):
    ...
    sri_response_order = fields.Selection([('1', 'No Enviado'),
                                          ('2', 'Enviado realizado'),
                                          ('5', 'Enviado rechazado')],
                                         default='1')
    typeDoc = fields.Selection([('1', 'Factura'),
                                ('2', 'Nota de Crédito')],
                                default='1',
                                string='Tipo Documento',
                                compute='setType', store=True)

    send_mail_client = fields.Boolean("Notificación correo", default=False, related="invoice_id.send_mail_client")
    access_key = fields.Char('Clave de Acceso', related="invoice_id.access_key")
    xml_sri_request_id = fields.Many2one("sri.xml.send", "SRI XML Request", related="invoice_id.xml_sri_request_id")
    invoice_number = fields.Char(string='Nro. Documento', related="invoice_id.invoice_number")
    ...
    def getAccessKey(self, pos_reference):
        order = self.env['pos.order'].search([('pos_reference', '=', pos_reference[0])], limit=1)
    ...
    def _config_establishment(self):
        ...
        config = self.env['invoice.point.emission'].search([('point_of_sale', '=', self.config_id.id)])
        return config
    ...
    def send_email(self, attachments=[]):
        ...
        if template:
            ...
            mail_sent = et_pool.send_mail(self.env.cr, 1, template.id, self.env.user.id, context={'attachment_ids': attachments})
        return True
    ...
    def action_send_invoice(self):
        ...
        for order in self:
            ...
            if not invoice.invoice_number:
                invoice.invoice_number = "{}-{}-{}".format(invoice.code_emission.number_establishment.number_establish,
