...
class ExportStockInfoReport(models.TransientModel):
    ...
    def generate_export_stockinfo_report(self, today, to_date, all_product_ids, warehouse_or_location):
        ...
        if self.report_wise == 'Warehouse':
            ...
            if not warehouse_ids:
                sql_stock = "SELECT id FROM stock_location WHERE id in {}".format('(' + str(warehouse_or_location or [0]).strip('[]') + ')')
    ...
    def add_headings(self, locations_or_warehouses, today, to_date, workbook, header_bold, header_title, ware_or_loc_header_style, blank_cell_style):
        ...
        new_workbook.write(4, 0, 'Estado', header_bold)
        new_workbook.write(4, 1, 'Proveedor', header_bold)
        new_workbook.write(4, 2, 'SKU', header_bold)
        new_workbook.write(4, 3, 'Barcode', header_bold)
        new_workbook.write(4, 4, 'Nombre', header_bold)
        new_workbook.write(4, 5, 'Categoria', header_bold)
        new_workbook.write(4, 6, 'Precio', header_bold)
        ...
        if len(locations_or_warehouses) == 1:
            ...
            for warehouse_or_location in locations_or_warehouses:
                col2 += 12
        new_workbook.write_merge(2, 2, 7, col2, string_warehouse_or_locations, ware_or_loc_header_style)
    ...
    def prepare_data(self, today, all_product_ids, warehouse_or_location):
        single_data_dict = {}
        if self.report_wise == "Warehouse":
            sql_stock = "SELECT id, view_location_id FROM stock_warehouse WHERE id in {}".format('(' + str(warehouse_or_location or [0]).strip('[]') + ')')
            self.env.cr.execute(sql_stock)
            ...
            sql = "SELECT id FROM stock_location WHERE id in {}".format('(' + str(warehouse_or_location or [0]).strip('[]') + ')')
    ...
    def get_data_ware_or_loc(self, child_locations_list, all_product_ids):
        sql_stock = "SELECT id FROM stock_location WHERE usage = '{}'".format('customer')
        self.env.cr.execute(sql_stock)
...
group by product order by product
    ...
    def get_child_locations(self, location):
        ...
        sql = "SELECT id FROM stock_location WHERE usage = '{}' and location_id = '{}'".format('internal', location)
        self.env.cr.execute(sql)
    ...
    def auto_generator_export_stockinfo_report(self):
        ...
        mail_template_view = self.env.ref('ec_11_export_stock.mail_template_export_stockinfo_report_ept')
    ...
    def print_stock_info_report_pdf(self):
        return self.env.ref('ec_11_export_stock.export_stockinfo_reprot_pdf_action').report_action(self)