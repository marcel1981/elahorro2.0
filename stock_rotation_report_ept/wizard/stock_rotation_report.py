...
class Stock_Rotation_Report(models.TransientModel):
    ...
    def prepare_data(self,from_date,to_date,warehouses,all_products):
        ...
        for warehouse in warehouses:
            ...
            for product in all_products:
                ...
                warehouse_in_qty = 0
                from_date  = from_date
                formated_date=datetime.strptime(from_date, "%Y-%m-%d")
                date_time = formated_date - timedelta(days=1)
                date_of_stock = datetime.strftime(date_time, "%Y-%m-%d")
                product_id=product.with_context(warehouse=warehouse.id,to_date=date_of_stock)
                opening_product_qty = product_id.qty_available
                ...
                if warehouse.id in data_dict:
                    ...
                    data_lst.append({'product':product,'sku':product.default_code or '','name':product.name,
                                     ...
                                     'adjusted_qty_in_duration':adjusted_qty_in_duration or 0,'warehouse_in_qty':warehouse_in_qty or 0,
    ...
    def prepare_date_for_all_warehouses_sheets(self,product,product_data_dict,opening_qty,last_sales,last_purchase_date,qty_purchase_in_duration,qty_sales_in_duration,scrap_location_qty,adjusted_qty_in_duration,warehouse_in_qty,warehouse_out_qty):
        ...
        if product in product_data_dict:
    ...
    def print_all_rotation_sheet_data(self,product_data_dict,workbook,worksheet_all_stock_rotation,body_style,qty_cell_style,value_style,blank_cell_style,value_style2):
        ...
        for product_id,product_data in product_data_dict.items():
            ...
            last_purchase_date = ''
            last_sales = ''
            last_purchase_date_lst = product_data.get('last_purchase_date')
            if any(last_purchase_date_lst):
                last_purchase_date_lst = filter(None, last_purchase_date_lst)
                last_purchase_date = max(last_purchase_date_lst)
                last_purchase_date = last_purchase_date.strftime('%d-%m-%Y') or ''
            last_sales_lst = product_data.get('last_sales')
            if any(last_sales_lst):
                last_sales_lst = filter(None, last_sales_lst)
