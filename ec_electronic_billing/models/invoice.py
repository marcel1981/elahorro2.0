...
class AccountInvoice(models.Model):
    ...
    identification = fields.Char(string='Identificación', related='partner_id.identification', readonly=True)
    msj_request = fields.Text(string='Mensaje Recepción', related='xml_sri_request_id.sri_receipt')
    msj_auth = fields.Text(string='Mensaje Authorización', related='xml_sri_request_id.sri_auth')
    local_tz = pytz.timezone('America/Guayaquil')#TODO
    ec_time = datetime.now(local_tz)
    date_order_local = fields.Datetime('Date current action', required=False, readonly=True, default=ec_time)
    ...
    physical_billing = fields.Boolean('Facturación física', default=False, readonly=False)
    creditNoteDocResource = fields.Many2one('account.invoice', 'Número Fact. del cliente a afectar')

    TEMPLATES = {
        'invoice': 'invoice.xml',
        'note_credit': 'credit_note.xml'
    }
    ...
    def action_send_invoice(self):
        ...
        for inv in self:
            ...
            self.invoice_number = "{}-{}-{}".format(self.code_emission.number_establishment.number_establish,
        ...
        if access_key:
            ...
            if state:
                if not self.send_mail_client:
                    ...
                    send_mail = self.send_email(self, attachments=[(4, a.id) for a in atts])
                    if send_mail:
    ...
    def invoice_print(self):
        if self.type_document() == 'invoice':
            return self.env.ref('ec_electronic_billing.account_invoices').report_action(self)
        elif self.type_document() == 'note_credit':
            return self.env.ref('ec_electronic_billing.account_credit_note').report_action(self)
    ...
    def create_attachment_pdf(self, name, invoice, model='account.invoice'):
        ...
        if invoice.type_document() == 'invoice':
            data, data_format = self.env.ref('ec_electronic_billing.account_invoices').sudo().render([invoice.id])
            ...
            data = request.env.ref('ec_electronic_billing.account_credit_note').sudo().render_qweb_pdf([invoice.id])[0]
    ...
    def create_envio(self, menssage, res):
        ...
        envio = {
            'sri_receipt': menssage,
            'sri_xml_response': res,
        }
    ...
    def send_email(self, invoice, attachments=[]):
        ...
        if attachments:
            ...
            body_html = '<p>Estimada(o) {}</p> <p>Muchas gracias por ser parte de la familia Almacenes El Ahorro, adjuntamos' \
                        ...
                        '<p>Saludos</p><p>Almacenes El Ahorro</p>'.format(invoice.partner_id.name)
            values = {
...
class InvoiceReport(models.AbstractModel):
    ...
    def invoice_print(self, docids, data=None):
        ...
        report = report_obj._get_report_from_name('ec_electronic_billing.report_invoice')
